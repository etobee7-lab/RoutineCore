...
new_code = r"""const ScheduleOnlyCalendar = ({ todos, startEdit, closeCalendar, userPoints, progress, userAvatar, currentUser, setShowSuccessRoom, setShowMyPage, setShowDailyChart, handleInstallClick, showInstallBtn, setIsAuthenticated, setCurrentUser }) => {
    ...
    <div className="calendar-mobile-optimized" style={{ 
      ...
      <div style={{ padding: '25px 20px' }}>
        <div style={{
          ...
          border: '1px solid rgba(255,255,255,0.06)', marginBottom: '25px'
        }}>
            ...
            return (
              ...
              >
                <div className="day-header-premium" style={{
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: '0.85rem', color: '#64748b', fontWeight: '700' }}>{dayTodos.length}개</span>
                </div>
                  ...
                  <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {dayTodos.map(todo => (
                      <div
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px 20px', borderRadius: '18px', background: 'rgba(255,255,255,0.02)', borderLeft: `5px solid ${color}`, cursor: 'pointer', transition: 'all 0.2s', border: '1px solid rgba(255,255,255,0.01)' }}
                      >
                        <span style={{ fontSize: '0.85rem', color: color, fontWeight: '900', minWidth: '70px' }}>{formatTime(todo.time)}</span>
                        <span style={{ fontSize: '1rem', color: '#f1f5f9', flex: 1, fontWeight: '600' }}>{todo.text}</span>
...
# Replace lines from start_line to end_line (1-indexed)
lines[start_line-1:end_line] = [new_code + '\n']

with open('src/App.jsx', 'w', encoding='utf-8') as f:
    f.writelines(lines)
print(f"Successfully optimized mobile view at lines {start_line}-{end_line}")
